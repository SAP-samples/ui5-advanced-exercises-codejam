...
export default class Supermarket extends Control {
  ...
  onAfterRendering(): void {
    ...
    loader.load("supermarket.glb", (gltf) => {
      ...
      const busyIndicator = this.getAggregation(
        "_busyIndicator"
      ) as BusyIndicator;
  ...
  setCameraPosition(
    ...
    { backToStart = false }: { backToStart?: boolean }
  ): void {
    if (backToStart) {
  ...
  expand({ stayExpanded = false }: { stayExpanded?: boolean }): void {
    ...
    const factor =
      icon === "sap-icon://full-screen" || stayExpanded ? growFactor : 1;
    // @ts-ignore
    ...
    expand.setIcon(
      `sap-icon://${factor === growFactor ? "exit-" : ""}full-screen`
    );
  }
